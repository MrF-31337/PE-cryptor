...
PE::PE(const string &fname)
{
	...
	for (int i = 0; i < file_header->NumberOfSections; i++)
	{
		...
		Section section(section_header, &data[section_header->PointerToRawData], false);
		for (int j = 0; j < 16; j++)
		{
...
void PE::add_section(const Section& section)
{
	sections.push_back(section);
...
void PE::resize_section(Section &section, uint new_size)
{
	...
	else if (section.header->SizeOfRawData < new_size)
	{
		...
		auto section_iterator = find_if(sections.begin(), sections.end(), [&section](Section &section1) {
...
void PE::save(const string& fname)
{
	...
	Section *last_section = sections_sorted_by_addr[sections_sorted_by_addr.size() - 1];
...
{
	sort(sections.begin(), sections.end(), [](Section& section1, Section& section2) {
		return section1.header->PointerToRawData < section2.header->PointerToRawData;
	});
}
